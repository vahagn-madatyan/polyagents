...
plan: 02
subsystem: database
tags: [python, filelock, json, persistence, atomic-write, state-recovery]

# Dependency graph
requires:
  - phase: 07-01
    provides: agents/utils/env.py with _env_bool/_env_int/_env_float shared helpers
  - phase: prior phases (01-06)
    provides: InGameTrader with in-memory _order_log and _ended_games state
provides:
...
affects: [08-state-persistence, 09-slug-normalization, any future InGameTrader consumer needing trade history]

# Tech tracking
tech-stack:
  added: []  # filelock was already a dependency via pregame_cache.py
  patterns:
    - "Atomic JSON write: tempfile.mkstemp + os.replace for crash-safe file updates"
    - "Graceful load: missing file -> empty state (no crash), corrupt file -> warning + empty state (no crash)"
    - "int->str->int round-trip: JSON dict keys always serialized as str, deserialized back to int on load"
    - "Separate lock per subsystem: SPORTS_STATE_LOCK_PATH distinct from SPORTS_BUDGET_LOCK_PATH to avoid deadlock"
