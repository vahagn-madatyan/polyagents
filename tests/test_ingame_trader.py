...
class TestHaltGate:
    ...
    def test_inprogress_proceeds_normally(self, monkeypatch):
        ...
        with _patch.object(trader, "_should_process", return_value=False) as mock_sp:
            trader._handle_score_change(1, prev, current, tag)
            mock_sp.assert_called_once_with(1, league="nba")
...
# ---------------------------------------------------------------------------
# TestPerSportCooldown
# ---------------------------------------------------------------------------


class TestPerSportCooldown:
    """VALID-02: Sport-specific debounce thresholds per league."""

    def test_global_cooldown_used_when_no_sport_override(self, monkeypatch):
        """Without SPORTS_INGAME_COOLDOWN_NBA, global value is used."""
        monkeypatch.setenv("SPORTS_INGAME_COOLDOWN_SECONDS", "30")
        trader, _ = _make_trader(monkeypatch=monkeypatch)

        assert trader.cooldown_seconds == 30
        assert "nba" not in trader._sport_cooldowns

        # Cooldown uses the global value
        trader._last_processed[1] = time.time()
        assert trader._is_in_cooldown(1, league="nba") is True

        trader._last_processed[1] = time.time() - 999
        assert trader._is_in_cooldown(1, league="nba") is False

    def test_sport_cooldown_loaded_from_env(self, monkeypatch):
        """SPORTS_INGAME_COOLDOWN_NBA=10 overrides the global 30s for NBA games."""
        monkeypatch.setenv("SPORTS_INGAME_COOLDOWN_SECONDS", "30")
        monkeypatch.setenv("SPORTS_INGAME_COOLDOWN_NBA", "10")
        trader, _ = _make_trader(monkeypatch=monkeypatch)

        assert trader._sport_cooldowns["nba"] == 10
        assert trader.cooldown_seconds == 30

    def test_multiple_sport_cooldowns(self, monkeypatch):
        """Multiple sport-specific cooldowns coexist."""
        monkeypatch.setenv("SPORTS_INGAME_COOLDOWN_SECONDS", "30")
        monkeypatch.setenv("SPORTS_INGAME_COOLDOWN_NBA", "10")
        monkeypatch.setenv("SPORTS_INGAME_COOLDOWN_NFL", "45")
        monkeypatch.setenv("SPORTS_INGAME_COOLDOWN_SOCCER", "5")
        trader, _ = _make_trader(monkeypatch=monkeypatch)

        assert trader._sport_cooldowns["nba"] == 10
        assert trader._sport_cooldowns["nfl"] == 45
        assert trader._sport_cooldowns["soccer"] == 5

    def test_sport_cooldown_shorter_than_global(self, monkeypatch):
        """NBA with 5s cooldown processes faster than 30s global."""
        monkeypatch.setenv("SPORTS_INGAME_COOLDOWN_SECONDS", "30")
        monkeypatch.setenv("SPORTS_INGAME_COOLDOWN_NBA", "5")
        trader, _ = _make_trader(monkeypatch=monkeypatch)

        # 10 seconds ago — within 30s global cooldown but past 5s NBA cooldown
        trader._last_processed[1] = time.time() - 10

        # With NBA league, should NOT be in cooldown (10 > 5)
        assert trader._is_in_cooldown(1, league="nba") is False

        # With NFL league (no override), still in cooldown (10 < 30)
        assert trader._is_in_cooldown(1, league="nfl") is True

    def test_sport_cooldown_longer_than_global(self, monkeypatch):
        """NFL with 60s cooldown is more conservative than global 30s."""
        monkeypatch.setenv("SPORTS_INGAME_COOLDOWN_SECONDS", "30")
        monkeypatch.setenv("SPORTS_INGAME_COOLDOWN_NFL", "60")
        trader, _ = _make_trader(monkeypatch=monkeypatch)

        # 40 seconds ago — past 30s global but within 60s NFL override
        trader._last_processed[1] = time.time() - 40

        assert trader._is_in_cooldown(1, league="nfl") is True
        assert trader._is_in_cooldown(1, league="nba") is False  # uses 30s global

    def test_should_process_passes_league_to_cooldown(self, monkeypatch):
        """_should_process with league uses sport-specific cooldown."""
        monkeypatch.setenv("SPORTS_INGAME_COOLDOWN_SECONDS", "30")
        monkeypatch.setenv("SPORTS_INGAME_COOLDOWN_NBA", "5")
        trader, _ = _make_trader(monkeypatch=monkeypatch)
        ...
        # NBA: 10 > 5 → can process
        assert trader._should_process(1, league="nba") is True
        # NFL: 10 < 30 → blocked
        assert trader._should_process(1, league="nfl") is False

    def test_none_league_falls_back_to_global(self, monkeypatch):
        """None league uses global cooldown."""
        monkeypatch.setenv("SPORTS_INGAME_COOLDOWN_SECONDS", "30")
        monkeypatch.setenv("SPORTS_INGAME_COOLDOWN_NBA", "5")
        trader, _ = _make_trader(monkeypatch=monkeypatch)

        trader._last_processed[1] = time.time() - 10
        assert trader._is_in_cooldown(1, league=None) is True  # 10 < 30

    def test_invalid_env_value_ignored(self, monkeypatch):
        """Non-integer SPORTS_INGAME_COOLDOWN_NBA value is silently ignored."""
        monkeypatch.setenv("SPORTS_INGAME_COOLDOWN_SECONDS", "30")
        monkeypatch.setenv("SPORTS_INGAME_COOLDOWN_NBA", "not_a_number")
        trader, _ = _make_trader(monkeypatch=monkeypatch)

        assert "nba" not in trader._sport_cooldowns

    def test_tick_uses_league_for_cooldown(self, monkeypatch):
        """tick() routes league from game state into cooldown check."""
        monkeypatch.setenv("SPORTS_INGAME_COOLDOWN_SECONDS", "30")
        monkeypatch.setenv("SPORTS_INGAME_COOLDOWN_NBA", "0")
        trader, mocks = _make_trader(monkeypatch=monkeypatch)
        ...
        # Establish baseline
        trader._prev_game_states[1] = _game_state(
            game_id=1, score_raw="5-3", home_score=5, away_score=3, league="nba"
        )

        # Mark as recently processed
        trader._last_processed[1] = time.time() - 1

        # With NBA cooldown=0, should still process
        gs_new = _game_state(
            game_id=1, score_raw="8-3", home_score=8, away_score=3, league="nba"
        )
        trader.tick({1: gs_new}, slug_table)

        # Fast-path triggered → cache.get called
        mocks["cache"].get.assert_called()
...
class TestOrderRate:
    """VALID-03: Rolling 60-second order-rate instrumentation."""

    def test_initial_order_rate_is_zero(self, monkeypatch):
        """Freshly constructed trader has zero order rate."""
        trader, _ = _make_trader(monkeypatch=monkeypatch)
        assert trader.get_order_rate() == 0
        assert trader._order_timestamps == []

    def test_record_order_timestamp_increments(self, monkeypatch):
        """Each call to _record_order_timestamp adds an entry and returns count."""
        trader, _ = _make_trader(monkeypatch=monkeypatch)

        count1 = trader._record_order_timestamp()
        assert count1 == 1

        count2 = trader._record_order_timestamp()
        assert count2 == 2

        assert trader.get_order_rate() == 2

    def test_old_timestamps_pruned(self, monkeypatch):
        """Timestamps older than 60s are pruned on next recording."""
        trader, _ = _make_trader(monkeypatch=monkeypatch)

        # Add a timestamp 90 seconds ago (outside window)
        old_ts = time.time() - 90
        trader._order_timestamps.append(old_ts)

        # Now record a new one — old one should be pruned
        count = trader._record_order_timestamp()
        assert count == 1  # only the new one

    def test_get_order_rate_prunes_stale(self, monkeypatch):
        """get_order_rate() prunes stale timestamps without adding new ones."""
        trader, _ = _make_trader(monkeypatch=monkeypatch)
        ...
        assert trader.get_order_rate() == 2  # only fresh ones

    def test_real_trade_increments_order_rate(self, monkeypatch, capsys):
        """Non-dry-run trade execution increments the order-rate counter."""
        monkeypatch.setenv("SPORTS_INGAME_MIN_CONFIDENCE_GAP", "0.10")
        trader, mocks = _make_trader(dry_run=False, monkeypatch=monkeypatch)
        ...
        assert trader.get_order_rate() == 1
        captured = capsys.readouterr()
        assert "metric=order_rate_60s count=1" in captured.out

    def test_dry_run_does_not_increment_order_rate(self, monkeypatch, capsys):
        """Dry-run trade does NOT increment order-rate counter."""
        monkeypatch.setenv("SPORTS_INGAME_MIN_CONFIDENCE_GAP", "0.10")
        trader, mocks = _make_trader(dry_run=True, monkeypatch=monkeypatch)
        ...
        assert trader.get_order_rate() == 0
        captured = capsys.readouterr()
        assert "metric=order_rate_60s" not in captured.out

    def test_slow_path_real_trade_increments_order_rate(self, monkeypatch, capsys):
        """Slow-path non-dry-run trade also increments order-rate counter."""
        monkeypatch.setenv("SPORTS_INGAME_MIN_CONFIDENCE_GAP", "0.10")
        trader, mocks = _make_trader(dry_run=False, monkeypatch=monkeypatch)
    ...
    def test_slow_path_dry_run_no_rate_increment(self, monkeypatch, capsys):
        """Slow-path dry-run does NOT increment order-rate counter."""
        monkeypatch.setenv("SPORTS_INGAME_MIN_CONFIDENCE_GAP", "0.10")
        trader, mocks = _make_trader(dry_run=True, monkeypatch=monkeypatch)
    ...
    def test_order_rate_emitted_as_structured_log(self, monkeypatch, capsys):
        """Order-rate metric is emitted as a structured log line."""
        monkeypatch.setenv("SPORTS_INGAME_MIN_CONFIDENCE_GAP", "0.10")
        trader, mocks = _make_trader(dry_run=False, monkeypatch=monkeypatch)

        tag = _market_tag()
        trader._fast_path(1, _game_state(game_id=1), tag)
        trader._fast_path(2, _game_state(game_id=2), tag)

        captured = capsys.readouterr()
        assert "metric=order_rate_60s count=1" in captured.out
        assert "metric=order_rate_60s count=2" in captured.out

    def test_multiple_trades_rolling_window(self, monkeypatch):
        """Multiple trades within 60s all counted; old ones expire."""
        trader, _ = _make_trader(monkeypatch=monkeypatch)

        # Simulate 5 trades spread across time
        now = time.time()
        trader._order_timestamps = [
            now - 70,  # expired
            now - 50,  # active
        ...
        assert trader.get_order_rate() == 3  # only the 3 active ones
